"""One isolated 500mb V2 c50 burst; existing evidence and runners stay unchanged."""
import argparse
import contextlib
import hashlib
import json
...
def run_burst(recorder, runtime, out):
    ...
    meta = {"size": "500mb", "concurrency": CONCURRENCY, "rounds": 1,
            ...
            "region": "us-west-2", "started_iso": shared.bench.utc_iso()}
    ...
    try:
        ...
        if errors or any(row is None for row in rows):
            raise RuntimeError("Incomplete burst")
    finally:
        meta["finished_iso"] = shared.bench.utc_iso()
        shared.original.save(out / "raw/500mb_c50.json", {
            "meta": meta, "requests": [r for r in rows if r is not None], "worker_errors": errors})
...
def run(out):
    prior = HERE / "results/coldstart_v2_c100_500mb_2026-09-11"
    old_meta = json.loads((prior / "run.json").read_text())
    old_dep = json.loads((prior / "deployments.json").read_text())
    request = json.loads((prior / "create_requests.json").read_text())["500mb"]
    request.update(agentRuntimeName="coldstart_v2_c50_500mb_" + uuid.uuid4().hex[:8],
                   clientToken=str(uuid.uuid4()), description="Temporary isolated 500mb V2 c50 retest")
    region, account = old_meta["region"], old_meta["account"]
    ...
    uri = request["agentRuntimeArtifact"]["containerConfiguration"]["containerUri"]
    ...
    try:
        ...
        runtime = {k: old_dep["runtimes"]["500mb"][k] for k in ["docker_size_bytes", "ecr_size_bytes"]}
        runtime.update(name=request["agentRuntimeName"], id=created["agentRuntimeId"],
                       arn=created["agentRuntimeArn"], create_response=created, image_digest=image["imageDigest"])
        ...
        time.sleep(180)
        summary = run_burst(recorder, runtime, out)
