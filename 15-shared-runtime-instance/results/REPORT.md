...
本报告验证多个真实用户共用一个 AgentCore `runtimeSessionId`、复用同一台 EC2
实例和同一 Agent 容器时的隔离正确性与容量边界。测试分为两类：

1. **短程任务**：每个用户只要求 Agent 返回 `pong`，单次约数秒；
2. **长程任务**：每个用户分两个阶段完成小型 Web 项目，持续约 5～10 分钟，
   包含文件创建、回读、修订、session resume 和宿主机产物验收。

核心结论：

| 结论 | 结果 |
|---|---|
| 多用户隔离 | 18/18 检查通过，文件和会话无串扰 |
| 短程请求 | 2～40 并发全部成功，共 164/164 请求成功 |
| 短程并发机制 | 16 个应用槽位并行，超过 16 的请求排队，以延迟换成功率 |
| 短程无排队测试 | 24 个活跃 Agent 可完成，32 个活跃 Agent 为 0/32 |
| 长程请求 | 2、8、16 并发全部 100% 成功 |
| 长程硬边界 | 24 并发两次跨实例复测均为 0/24 成功 |
| 长程稳定建议 | 8～12 并发 |
| 长程已验证硬上限 | 16 并发 |
| 当前安全配置 | `MAX_PARALLEL_AGENTS=16` |

短程和长程结果并不矛盾。短程 40 并发测试仍受 16 槽 semaphore 保护，多余请求在
应用层排队；长程 24 并发测试则临时将 semaphore 放开到 24，使 24 个 Claude
子进程同时驻留，最终触发可重复的实例用户态失活。
