...
class Benchmark:
    ...
    def invoke(self, sid, phase, index, hold=0, release=None):
        ...
        try:
            ...
            if not body.get("process_id") or not body.get("boot_id"):
    ...
    def summarize(self):
        ...
        for phase in ("cold", "warmup", "warm", "scale"):
            ...
            phases[phase] = {
                ...
                "unique_processes": len({r["body"]["process_id"] for r in good}),
        ...
        cold_ok = (cold["successes"] == self.settings["cold_samples"] and cold["failures"] == 0
                   and cold["unique_processes"] == self.settings["cold_samples"] and cold["all_first_request"])
        ...
        warm_ok = (warmup is not None and warm["successes"] == self.settings["warm_samples"]
                   and warm["failures"] == 0
                   and all(r["body"]["process_id"] == warmup["body"]["process_id"]
        ...
        scale_ok = (scale["successes"] == self.settings["concurrency"] and scale["failures"] == 0
                    and scale["all_first_request"] and scale["unique_processes"] == self.settings["concurrency"]
        ...
        tests["7.4"] = {"status": "PASS" if scale_ok else "FAIL", "samples": scale["attempts"],
                        ...
                        "unique_processes": scale["unique_processes"],
