...
def main():
    ...
    parser.add_argument("--key-prefix", default="results")
    ...
    def publish():
        ...
        s3.put_object(Bucket=cfg["bucket"], Key=args.key_prefix + "/progress.json", Body=json.dumps(status).encode())
    ...
    try:
        ...
        for key, runtime in cfg["runtimes"].items():
            ...
            publish()
            settle = max(0, max(runtime["ready_observed_unix"] + cfg["settle_seconds"],
                                 previous_finished + args.interval_seconds) - time.time())
            if settle:
                time.sleep(settle)
        ...
        for name in ("benchmark.log", args.key_prefix + ".log", "client_checks.log", "build.log",
                     "build_config.json", args.config):
            if (ROOT / name).exists():
                shutil.copy2(ROOT / name, out / name)
        archive = ROOT / (args.output + ".tar.gz")
        with tarfile.open(archive, "w:gz") as stream:
            stream.add(out, arcname=args.output)
        s3.upload_file(str(archive), cfg["bucket"], args.key_prefix + "/results.tar.gz")
        s3.put_object(Bucket=cfg["bucket"], Key=args.key_prefix + "/final.json", Body=json.dumps(status).encode())
    return rc
