...
| --- | --- | --- | --- | --- | --- | --- |
| 7.1 | 冷启动基线：单实例首次请求延迟 P50 | 🔴 P0 | < 3s | ✅ 通过 | River | 单位 s，数值按宁夏/北京排列。串行基线（每区 100 次）：1.623/1.737；8 进程连续三轮：10 并发 1.761/1.730、50 并发 1.702/1.690、100 并发 1.688/1.694；冷却 180s 后独立 100 并发：1.971/2.208。并发 P50 仅统计成功请求，100 并发存在新会话创建限流。 |
| 7.2 | 冷启动基线：单实例首次请求延迟 P99 | 🔴 P0 | < 5s | ✅ 通过 | River | 单位 s，数值按宁夏/北京排列。串行基线（每区 100 次）：2.008/2.196；8 进程连续三轮：10 并发 2.048/2.111、50 并发 2.423/2.375、100 并发 2.824/2.200；冷却 180s 后独立 100 并发：3.003/3.192。并发 P99 仅统计成功请求，100 并发存在新会话创建限流。 |
