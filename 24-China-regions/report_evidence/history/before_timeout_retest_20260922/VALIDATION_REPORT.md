...
**资源状态补充复核：2026-09-22 07:06 UTC**

## 1. 验收结论

原始清单共 **13 项**：Code Interpreter 9 项、Runtime 4 项。
其中 **12 项在本报告注明的测试范围或配置条件下通过，1 项部分通过**。
部分通过项为 **2.6 执行超时**，因此不能判定全部 P0 验收完成。
...
- 两区均未在 180 秒观察中触发原生单次执行自动超时。
  “调用端期限 + `stopTask`”的取消路径有效，但不能替代原生执行超时验收。
...
| 2.6 | 执行超时机制有效（超时后任务正确终止） | 🔴 P0 | 🟡 部分通过 | River | 两区 180 秒任务均正常完成，未证明原生自动执行超时；5 秒调用端期限后 stopTask 成功，计数停止、任务 canceled、后续执行正常。 |
...
**2.6 保持部分通过。** 需要明确内部测试版本的原生执行上限或可配置参数，
再按该期限验证“超时返回后任务不再继续运行”。
...
| --- | --- | --- | --- |
| F1 | 原生执行超时 | 2.6 尚未关闭。确认服务端上限或参数，补测自动超时后的任务终止与恢复。 | River |
...
本次补核仅更新报告证据，没有重启实例、重复压测或修改历史测试结果。
