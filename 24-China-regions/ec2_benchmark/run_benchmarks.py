...
def main():
    ...
    assert identity["region"] == settings["region"], identity
    ...
    sources.mkdir()
    source_names = ["run_benchmarks.py", "benchmark_runtime.py", "verify_code_interpreter.py",
                    "config.json", "requirements.txt"]
    if settings.get("efs"):
        source_names += ["verify_efs.py", "efs_resources.json"]
    ...
    try:
        ...
        try:
            ...
            if settings.get("run_functional"):
                with state_lock:
                    state["phase"] = "code_interpreter_functional"
                ci.functional()
                with state_lock:
                    state["phase"] = "code_interpreter_timeout"
                ci.timeout()
                results["ci_functional"] = ci.results
        finally:
            ci.cleanup()
        if settings.get("public_interpreter_id"):
            with state_lock:
                state["phase"] = "code_interpreter_public_dependencies"
            public_dir = DATA / "code_interpreter_public"
            public_args = SimpleNamespace(output=str(public_dir), profile=None, region=settings["region"],
                expected_account=settings["account"], identifier=settings["public_interpreter_id"],
                observation_seconds=180, concurrency=[1, 10, 50])
            public = CodeLab(public_args)
            try:
                public.environment()
                public.dependencies()
                results["ci_public"] = public.results
            finally:
                public.cleanup()
        if settings.get("efs"):
            with state_lock:
                state["phase"] = "code_interpreter_efs"
            from verify_efs import Lab as EfsLab
            efs_dir = DATA / "efs"
            efs_dir.mkdir()
            shutil.copyfile(BASE / "efs_resources.json", efs_dir / "resources.json")
            efs = EfsLab(efs_dir, region=settings["region"],
                         subnet_id=settings["efs"]["subnet_id"], profile_name=None)
            try:
                efs.verify()
                results["efs"] = json.loads((efs_dir / "result.json").read_text())
            except Exception as exc:
                results["efs"] = {"status": "FAIL", "error": str(exc)}
                (efs_dir / "failure.txt").write_text(traceback.format_exc())
            finally:
                for session in efs.state["sessions"]:
