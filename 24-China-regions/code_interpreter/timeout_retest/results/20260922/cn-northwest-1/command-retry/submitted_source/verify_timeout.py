...
child = None
if role == "parent":
    child = subprocess.Popen([sys.executable, __file__, "child", mode, str(folder), str(duration)])
started = time.monotonic()
counter = 0
...
def main():
    ...
    prefix = cfg.get("result_prefix", "results")
    lab = Lab(cfg)
    save(OUT / "config.json", cfg)
    save(OUT / "source.json", {"sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest()})
    completed = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        futures = ({} if args.command_only else
                   {pool.submit(lab.native_ttl, mode): "ttl-" + mode for mode in ("executeCode", "async")})
        ...
        try:
            while futures:
                ...
                lab.s3.put_object(Bucket=cfg["bucket"], Key=prefix + "/status.json",
    ...
    lab.s3.upload_file(str(archive), cfg["bucket"], prefix + "/results.tar.gz")
    lab.s3.put_object(Bucket=cfg["bucket"], Key=prefix + "/final_status.json",
